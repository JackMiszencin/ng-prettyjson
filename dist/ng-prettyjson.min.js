...
* License: MIT 
**/
!function(a){"use strict";a.module("ngPrettyJson",[]).directive("prettyJson",["$compile","$templateCache","ngPrettyJsonFunctions",function(b,c,d){var e=a.isDefined;return{restrict:"AE",scope:{json:"=",prettyJson:"=",settings:"=",onEdit:"&"},template:"<div></div>",replace:!0,link:function(f,g,h){var i={},j=null;f.id=h.id||"prettyjson",f.settings=f.settings||{},f.edition=h.edition,f.aceEditor=void 0!==window.ace;
// compile template
...
// trigger update
f.$apply(function(){})};f.edit=function(){return f.aceEditor?(f.settings.editActivated?(j&&(document.getElementById(f.id).env=null),n(i)):(j=ace.edit(f.id),j.setAutoScrollEditorIntoView(!0),j.setOptions({maxLines:1/0}),j.on("change",o),j.getSession().setMode("ace/mode/json")),void(f.settings.editActivated=!f.settings.editActivated)):void(console&&console.log("'ace lib is missing'"))},f.update=function(){f[m]=i,f.$emit("json-updated",i),f.onEdit&&f.onEdit({newJson:i}),this.edit()}}}}]).factory("ngPrettyJsonFunctions",function(){
