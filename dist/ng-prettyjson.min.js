...
* License: MIT 
**/
!function(a){"use strict";a.module("ngPrettyJson",[]).directive("prettyJson",["$compile","$templateCache","ngPrettyJsonFunctions",function(b,c,d){var e=a.isDefined;return{restrict:"AE",scope:{json:"=",prettyJson:"=",settings:"=",triggers:"=",onEdit:"&"},template:"<div></div>",replace:!0,link:function(f,g,h){var i={},j=null;f.id=h.id||"prettyjson",f.settings=f.settings||{},f.triggers=f.triggers||{},f.edition=h.edition,f.aceEditor=void 0!==window.ace,f.showEditButton=e(f.settings.showEditButton)?f.settings.showEditButton:!0;
// compile template
...
a.isObject(b)&&e(b.json)?(l(),f.$watch(m+".json",function(a){f.settings.editActivated||n(a),i=a},!0)):(f.settings.editActivated||n(b),i=b),j&&(j.removeListener("change",o),j.setValue(JSON.stringify(b,null,"	")),j.on("change",o),j.resize())},!0);var o=function(a){try{i=JSON.parse(j.getValue()),f.parsable=!0}catch(b){f.parsable=!1}
// trigger update
f.$apply(function(){})};f.edit=function(){return f.aceEditor?(f.settings.editActivated?(j&&(document.getElementById(f.id).env=null),n(i)):(j=ace.edit(f.id),j.setAutoScrollEditorIntoView(!0),j.setOptions({maxLines:1/0}),j.on("change",o),j.getSession().setMode("ace/mode/json")),void(f.settings.editActivated=!f.settings.editActivated)):void(console&&console.log("'ace lib is missing'"))},
// Allows parent scope to access edit function
f.triggers.edit=f.edit,f.update=function(){f[m]=i,f.$emit("json-updated",i),f.onEdit&&f.onEdit({newJson:i}),this.edit()},f.triggers.update=f.update}}}]).factory("ngPrettyJsonFunctions",function(){
